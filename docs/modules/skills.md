...
| File | Owner | Notes |
|---|---|---|
| `advice.json` | supervisor Brain | state, advice, max_new_agents, stopped quadrants, notes |
...
`brain_list_all_quadrants`, `brain_list_brains`, `brain_get_project` (advise
mode), `brain_send_to_brain` (advise mode), `chat_say`. Deliberately not
